...
router.get('/',verifyAccessToken, function(req, res, next) {
  ...
  files.forEach(file => {
    if (file.match('^[0-9]{8}-[0-9]{5}')){
      ...
      dat=matter(dat);
      let contentLine = {};
      contentLine.file = file;
      contentLine.title = dat.data.Title;
      contentLine.type = dat.data.type;
      contentArr.push(contentLine);
    } 
  });

  res.render('index',{content:contentArr,access_granted:req.access_granted});
});
...
router.get('/:i',verifyAccessToken, function(req, res, next) {
  if ( req.params.i.match('^[0-9]{8}-[0-9]{5}') ){
    try {
      ...
      res.render('single',{file: req.params.i,data: dat.data,content: md.render(dat.content),access_granted:req.access_granted});
    }
    catch (err){
